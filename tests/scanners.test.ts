...
describe("scanFile", () => {
  ...
  describe("severity", () => {
    it("marks env files as critical", () => {
      ...
      expect(
        findings.find((f) => f.rule === "env-file-committed")?.severity
      ).toBe("critical");
    });
    ...
    it("marks secrets as critical", () => {
      ...
      expect(
        findings.find((f) => f.rule === "hardcoded-secret")?.severity
      ).toBe("critical");
    });

    it("marks internal IPs as medium", () => {
      const findings = scanFile("config.ts", 'const db = "192.168.1.1"');
      expect(findings.find((f) => f.rule === "internal-ip")?.severity).toBe(
        "medium"
      );
    });
  });
